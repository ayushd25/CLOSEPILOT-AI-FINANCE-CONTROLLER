...
def _tax(rng: random.Random) -> SyntheticCase:
    ...
    payment = _make_record(rng, RecordType.PAYMENT, pay_id, amt, transaction_at=ts)
    settlement = _make_record(rng, RecordType.SETTLEMENT, set_id, amt - tax, transaction_at=ts + timedelta(days=1), metadata={"tax": tax})
    bank = _make_record(rng, RecordType.BANK_TRANSACTION, bank_id, amt - tax, transaction_at=ts + timedelta(days=1))

    tax_record = _make_record(rng, RecordType.TAX, f"tax_{rng.randint(100000, 999999)}", tax, transaction_at=ts + timedelta(days=1))

...
ClosePilot operates entirely on **controlled synthetic data** generated by an internal scenario generator. It produces payments, settlements, and bank transactions across 20 reconciliation scenarios (exact matches, splits, refunds, duplicates, fee/tax discrepancies, conflicting candidates, suspicious activity, and more) along with **hidden ground truth** used only by the evaluation lab — never exposed to the LLM.
...
2. **Sources** (`/sources`) — generate synthetic data (choose case count and seed); view generated datasets.
...
A typical demo flow: **Sources → generate ~100 cases → Reconciliation → run → Exceptions → investigate → Evidence → Evaluation → Audit**.
